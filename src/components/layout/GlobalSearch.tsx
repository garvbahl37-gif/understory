...
const LABEL_COLOR: Record<string, string> = {
  Service: "var(--chalk)",
  Package: "var(--strata-2)",
  Advisory: "var(--sev-high)",
  Maintainer: "var(--lichen)",
};
  ...
  return (
    <>
      <button
        type="button"
        onClick={openDialog}
        className="flex items-center gap-2 rounded border border-rule bg-[var(--peat-sunken)] px-2.5 py-1.5 text-[12px] text-lichen transition-colors hover:border-rule-strong hover:text-bone-dim"
      >
        ...
        <kbd className="u-mono ml-1 hidden rounded border border-rule px-1 py-px text-[9.5px] text-lichen-dim sm:inline">
          ⌘K
        </kbd>
      </button>
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 border-b border-rule px-4 py-3">
              <svg width="13" height="13" viewBox="0 0 12 12" aria-hidden fill="none" className="text-lichen">
                <circle cx="5" cy="5" r="3.6" stroke="currentColor" strokeWidth="1.3" />
                <path d="M7.8 7.8 L11 11" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
              </svg>
              <input
                ...
                className="w-full bg-transparent text-[14px] text-bone outline-none placeholder:text-lichen-dim"
                aria-label="Search the graph"
              />
              {loading ? <span className="breathe u-mono text-[10px] text-lichen-dim">…</span> : null}
            </div>

            <div className="max-h-[52vh] overflow-y-auto">
              {results.length === 0 && !loading ? (
                <p className="px-4 py-6 text-[13px] text-lichen">
                  {query
              ...
              {results.map((candidate, index) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                    index === cursor ? "bg-[var(--peat-high)]" : ""
                  }`}
                >
                  <span
                    className="u-mono w-[74px] shrink-0 text-[9.5px] uppercase tracking-[0.11em]"
                    style={{ color: LABEL_COLOR[candidate.label] ?? "var(--lichen)" }}
                  >
                    {candidate.label}
                  </span>
                  <span className="u-mono min-w-0 flex-1 truncate text-[12.5px] text-bone">
                    {candidate.caption}
                  </span>
                  {candidate.sub ? (
                    <span className="u-mono shrink-0 text-[10.5px] text-lichen-dim">{candidate.sub}</span>
                  ) : null}
                </button>
              ))}
            </div>
