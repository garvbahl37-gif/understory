      ...
      <Panel className="mb-6">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {report.latencyMs !== undefined ? (
            <span className="u-num text-[13px] text-lichen">{report.latencyMs}ms round trip</span>
          ) : null}
        </div>
        ...
        <dl className="mt-5 grid gap-4 border-t border-rule pt-4 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <dd className="u-mono mt-1.5 break-all text-[12px] text-bone-dim">{report.target}</dd>
          </div>
          <div>
            <dt className="u-eyebrow">Database</dt>
            <dd className="u-mono mt-1.5 text-[12px] text-bone-dim">{report.database}</dd>
          </div>
          <div>
            <dt className="u-eyebrow">Server</dt>
            <dd className="u-mono mt-1.5 break-all text-[12px] text-bone-dim">
              {report.server?.agent ?? "—"}
            </dd>
          </div>
          <div>
            <dt className="u-eyebrow">Bolt protocol</dt>
            <dd className="u-mono mt-1.5 text-[12px] text-bone-dim">
              {report.server?.protocolVersion ?? "—"}
            </dd>
          </div>
        </dl>

        <p className="mt-4 border-t border-rule pt-4 text-[12px] text-lichen">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <Panel>
            ...
            <p className="text-[13px] leading-relaxed text-bone-dim">
              ...
              <code className="u-mono text-[12px] text-chalk">globalThis</code> and reused across warm
